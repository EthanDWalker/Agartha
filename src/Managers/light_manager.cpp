...
void LightManager::Init(DescriptorBuilder &descriptor_builder) {
  CreateBuffer(sizeof(glm::mat4) * MAX_LIGHT_MATRICES,
               VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
               VMA_MEMORY_USAGE_GPU_ONLY, matrix_buffer);

  CreateBuffer(sizeof(DirectionalLight) * MAX_DIRECTIONAL_LIGHTS,
               VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
               VMA_MEMORY_USAGE_GPU_ONLY, directional_light_buffer);
  CreateBuffer(sizeof(PointLight) * MAX_POINT_LIGHTS,
               VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
  ...
  descriptor_builder.Build(VK_SHADER_STAGE_ALL, light_descriptor_set, light_descriptor_layout);
  ...
  vkCreateSampler(VulkanContext::device, &shadow_sampler_ci, nullptr, &shadow_sampler);
  ...
  descriptor_builder.Build(VK_SHADER_STAGE_ALL, shadow_descriptor_set, shadow_descriptor_layout);
}

uint32_t LightManager::AddDirectionalLight(glm::vec3 color, glm::vec3 direction,
                                           float intensity) {
  ...
  UpdateBuffer(&dir_light, sizeof(DirectionalLight),
               sizeof(DirectionalLight) * directional_light_index, directional_light_buffer);

  glm::vec3 light_dir = glm::normalize(direction);
  glm::vec3 light_pos = glm::zero<glm::vec3>() - light_dir * DIRECTIONAL_LIGHT_DISTANCE;
  ...
  for (uint32_t i = 0; i < CASCADES.size(); i++) {
    ...
    CreateAllocatedImage(SHADOW_IMAGE_EXTENT, VK_FORMAT_D32_SFLOAT,
  ...
  UpdateBuffer(light_matrices.data(), sizeof(glm::mat4) * light_matrices.size(),
...
void LightManager::UpdateDirectionalLight(glm::vec3 direction, uint32_t index ) {
  direction = glm::normalize(direction);
  assert(index <= directional_lights.size() && "attempt to update out of bounds directional light");
  directional_lights[index] = direction;
  uint32_t direction_offset = offsetof(DirectionalLight, direction);
  UpdateBuffer(&direction, sizeof(direction),
               (index * sizeof(DirectionalLight)) + direction_offset, directional_light_buffer);
}

void LightManager::UpdateMatrices( glm::vec3 camera_position) {
  ...
  UpdateBuffer(matrices.data(), sizeof(glm::mat4) * matrices.size(), 0,
               matrix_buffer);
}

uint32_t LightManager::AddPointLight(glm::vec3 color, glm::vec3 position, float intensity) {
  PointLight point_light{};
  point_light.position = position;
  point_light.color = color;
  point_light.intensity = intensity;

  UpdateBuffer(&point_light, sizeof(PointLight), sizeof(PointLight) * point_light_index,
               point_light_buffer);
...
void LightManager::Destroy() {
  ...
  DestroyImageSampler(shadow_sampler);

  vkDestroyDescriptorSetLayout(VulkanContext::device, light_descriptor_layout, nullptr);
  vkDestroyDescriptorSetLayout(VulkanContext::device, shadow_descriptor_layout, nullptr);

  DestroyBuffer(matrix_buffer);
  DestroyBuffer(point_light_buffer);
  DestroyBuffer(directional_light_buffer);
}
