...
void TransformationWidget::Create(DescriptorBuilder &descriptor_builder, Camera &camera,
  ...
  CreateBufferData(mesh_data.vertices.data(), sizeof(Vertex) * mesh_data.vertices.size(),
  ...
  CreateBufferData((void *)DIRECTION_INSTANCES, sizeof(glm::mat4) * TRANSFORMATION_DIRECTION_COUNT,
                   VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, instance_buffer);
  CreateBufferData((void *)DIRECTION_COLORS, sizeof(glm::vec4) * TRANSFORMATION_DIRECTION_COUNT,
  ...
  descriptor_builder.Build(VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                           descriptor_set, descriptor_layout);

  GraphicsPipelineBuilder pipeline_builder{};
  pipeline_builder.SetShaders("Debug/widget.vert.spv", "Debug/widget.frag.spv");
...
void TransformationWidget::Update(Camera &camera, glm::vec2 mouse_pos) {
  if (matrix == glm::mat4(0.0f))
    return;
  if (InputContext::GetInputPressed(Input::ONE)) {
  ...
  case (TransformationMode::MOVE): {
    ...
    matrix[3][selected_direction] = intersection[selected_direction] -
  ...
  case (TransformationMode::ROTATE): {
    ...
    case (TransformationDirections::X): {
      rotation_matrix =
          glm::mat3(glm::vec3(1, 0, 0), glm::vec3(0, cos, -sin), glm::vec3(0, sin, cos));
      break;
    }
    case (TransformationDirections::Y): {
      rotation_matrix =
          glm::mat3(glm::vec3(cos, 0, sin), glm::vec3(0, 1, 0), glm::vec3(-sin, 0, cos));
      break;
    }
    case (TransformationDirections::Z): {
      rotation_matrix =
          glm::mat3(glm::vec3(cos, -sin, 0), glm::vec3(sin, cos, 0), glm::vec3(0, 0, 1));
      break;
    }
    default: {
      return;
  ...
  default: {
    assert(0 && "Must set translation mode to one the the enum values as defined in "
...
void TransformationWidget::Draw(VkCommandBuffer cmd, Camera &camera) {
  ...
  vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, draw_pipeline.layout, 0, ds.size(),
                          ds.data(), 0, nullptr);
...
void TransformationWidget::Destroy() {
  DestroyPipeline(draw_pipeline);
