...

  {
    ...
    builder.AddPass(1, main_pass_dep.dependency, [&](VkCommandBuffer cmd) {
...

      {
        ...
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                mesh_pipeline.layout, 0, ds.size(), ds.data(),
                                0, nullptr);
...
void Engine::Init() {
  ...
  ui_manager.Create(context, VK_FORMAT_R16G16B16A16_SFLOAT,
                    VK_FORMAT_D32_SFLOAT, window);
  ...
  /*{
    ...
    GraphicsPipelineBuilder pipeline_builder;
    pipeline_builder.Default();
    pipeline_builder.SetShaders(context, "shadow.vert.spv",
  "shadow.frag.spv"); pipeline_builder.SetDepthFormat(shadow_image.format);
