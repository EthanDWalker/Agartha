...
void SceneSvo::Create(VulkanContext &context, SceneManager &scene_manager,
  ...
  {
    for (uint32_t i = 0; i < SVO_DEPTH; i++) {
      descriptor_builder.BindStorageBuffer(i, levels[i].buffer);
    }
    descriptor_builder.BindUniformBuffer(SVO_DEPTH, data_buffer.buffer);
    ...
    pipeline_builder.SetViewportCount(3);
  ...
  {
    GraphicsPipelineBuilder pipeline_builder{};
    pipeline_builder.SetShaders(context, "VoxelGI/debug_view.vert.spv",
                                "VoxelGI/debug_view.frag.spv");
    pipeline_builder.Default();
...
void SceneSvo::DrawDebugView(VkCommandBuffer cmd, SceneManager &scene_manager,
