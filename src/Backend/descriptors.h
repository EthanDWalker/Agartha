...
struct DescriptorPool {
  static constexpr std::array<std::pair<VkDescriptorType, float>, 7> pool_ratios{{
  ...
  void Allocate(VkDescriptorSetLayout &layout, VkDescriptorSet &set, void *pNext = nullptr);
...
struct DescriptorBuilder {
  ...
  void BindCombinedImage(uint32_t binding, VkImageView image_view, VkSampler sampler);
  ...
  void BindAccelerationStructure(uint32_t binding, VkAccelerationStructureKHR &as);

  void Build(VkShaderStageFlags stage_flags, VkDescriptorSet &set, VkDescriptorSetLayout &layout);
